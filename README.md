...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-29: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture, real bert-tiny directly from Hugging Face's own export, and combined Q/K/V projections (GPT-2's real `c_attn` shape); causal/autoregressive masking supported throughout; GPT-2's own additive-bias mask decomposition is the one piece still standing between this and compiling real GPT-2 with zero rewiring)

...
31. [Real GPT-2's full 12-layer stack](research/experiments/exp31-real-gpt2-full-stack/results.md) — does real depth surface anything exp23's stacking fix didn't already cover, now that every per-layer GPT-2 quirk is closed? No: all 900 ONNX nodes of real GPT-2's complete decoder stack compile with **zero changes to the compiler**, matching its real output to 1.83e-04. The cleanest result in the whole GPT-2 arc (exp28-31) — nothing to fix, nothing to route around
32. [Token and position embeddings via Gather](research/experiments/exp32-token-position-embeddings/results.md) — the compiled model's own external input can now be raw token ids (int64), not just pre-computed float features, for the first time since exp15. Found that position embeddings usually constant-fold away entirely at a fixed sequence length (no `Gather` at all), and that forcing the real `Gather`-based computation makes combining embeddings with the transformer stack in one graph explode to 2400+ dynamic-shape-bookkeeping nodes — out of scope, so validated instead as two independently compiled artifacts (embeddings, then exp31's stack) chained together, matching real GPT-2's true end-to-end output to 1.83e-04, genuinely "raw token ids in, real hidden states out"
...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-32: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture, real bert-tiny, real GPT-2's complete 12-layer decoder stack, and (as a separately-compiled, chained artifact) its real token/position embedding layer — genuine "raw token ids in, real hidden states out"; still no `lm_head`, no single-graph dynamic-shape support, and only the specific ONNX shapes actually observed in real exports so far)

...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-23: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture (single or stacked) with no measurable performance cost vs. hand-written code; the attention pattern matcher is now tensor-flow-traced rather than shape-scanned, after stacking exposed a real bug in the shape-scan approach; still needs wider ONNX opset/op coverage to accept arbitrary real-world exports as-is)

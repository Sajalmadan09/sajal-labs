...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-27: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture, prajjwal1/bert-tiny's real 2-layer encoder directly from Hugging Face's own unmodified export, and causal/autoregressive attention masking; not yet tried against a real decoder model)

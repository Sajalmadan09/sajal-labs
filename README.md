...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-31: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture, real bert-tiny, and real GPT-2's complete 12-layer decoder stack, all traced directly from Hugging Face's own unmodified export with zero rewiring; still no token/position embeddings or `lm_head` — only the decoder-stack level — and only the specific ONNX shapes actually observed in real exports so far)

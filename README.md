...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-26: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture and prajjwal1/bert-tiny's real 2-layer encoder directly from Hugging Face's own unmodified export, matching its real output to 2.86e-06; causal masking and non-encoder-only architectures not yet attempted)

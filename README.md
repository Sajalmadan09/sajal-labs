...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-28: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture, real bert-tiny directly from Hugging Face's own export, and a real GPT-2 decoder block (weights real, Q/K/V rewired around a not-yet-supported combined-projection `Split`); causal/autoregressive masking supported throughout)

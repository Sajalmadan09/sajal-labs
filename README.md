...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-24: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture (single or stacked) with no measurable performance cost vs. hand-written code, accepting ONNX opset 17-20 exports of every architecture tested; not yet tried against a real, externally-published model rather than this project's own test models)

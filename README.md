...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-21: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture in full (4-head attention + residual + FFN), bit-identical to hand-written references throughout, zero compiler changes needed to combine capabilities validated in isolation; multi-block stacking and benchmarking the compiled artifact itself not yet attempted)

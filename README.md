...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-25: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture and, with real pretrained weights rewired into its known attention convention, prajjwal1/bert-tiny, bit-identical to hand-written references throughout; does not yet accept Hugging Face's own traced export as-is — MatMul+Add-style Linear and 4D-batched attention are known, named gaps)

...
11. Model compiler (ONNX → native C++, not hand-porting) — in progress (exp15-22: [python/sajal_compile.py](python/sajal_compile.py), compiles exp2's actual `TransformerBlock` architecture in full, bit-identical to hand-written references throughout, and now benchmarked directly against exp2's own hand-written binary — cold-start, warm-loop, RSS, and binary size all statistically indistinguishable; multi-block stacking not yet attempted)
